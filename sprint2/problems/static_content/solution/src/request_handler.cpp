...
namespace http_handler {
    ...
    std::variant<RequestHandler::FileResponse, RequestHandler::FileResponseErrors> RequestHandler::MakeFileResponse(fs::path path) const
    {
        ...
        // ћетод prepare_payload заполн€ет заголовки Content-Length и Transfer-Encoding
        // в зависимости от свойств тела сообщени€
        res.prepare_payload();
        r = std::move(res);
        return r;
    }
    ...
    bool RequestHandler::IsSubPath(fs::path path, fs::path base) const {
        // ѕриводим оба пути к каноничному виду (без . и ..)
        path = fs::weakly_canonical(path);
        base = fs::weakly_canonical(base);

        // ѕровер€ем, что все компоненты base содержатс€ внутри path
    ...
    std::variant<RequestHandler::StringResponse, RequestHandler::FileResponse> RequestHandler::HandleRequest(StringRequest&& req, const model::Game& gm) const {
        ...
        // «десь можно обработать запрос и сформировать ответ, но пока всегда отвечаем: Hello
    }
}  // namespace http_handler
