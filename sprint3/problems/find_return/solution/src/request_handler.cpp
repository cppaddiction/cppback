...
namespace http_handler {
    ...
    std::string ApiHandler::GameState(json::Builder& builder, const std::vector<model::Dog>& dogs, const std::unordered_map<std::uint64_t, model::LostObject>& lost_objects) const
    {
        ...
        for (const auto& dog : dogs)
        {
            ...
            helper.Key(std::to_string(dog.GetId())).Value(helpbuilder.StartDict().Key(NAME).Value(dog.GetName()).Key(POS).Value(pos).Key(SPEED).Value(speed).Key(DIR).Value(dog.GetDir()).Key(BAG).Value(bag).EndDict().Build().AsMap());
        }
        ...
        else if (str_form == GAME_TICK_WITH_SLASH || str_form == GAME_TICK_WITHOUT_SLASH)
        {
            ...
            if (req.method() == http::verb::post)
            {
                try {
                    ...
                    sm_.UpdateAllSessions(time_str, lg_);
    ...
    void ApiHandler::Tick(std::uint64_t time) const {
